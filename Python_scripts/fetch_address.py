...
geolocator = Nominatim(user_agent="otodom_project__analysis")
    ...
    try:
        query = """ SELECT RN, concat(latitude,',',longitude) as LOCATION
                    FROM (SELECT RN
                        ...
                        FROM otodom_data_short_flatten where rn between 1 and 4
                        ORDER BY rn  ) """
        ...
        pandas_df.to_sql('otodom_data_short_flatten_address', con=engine, if_exists='append', index=False, chunksize=16000, method=pd_writer)
